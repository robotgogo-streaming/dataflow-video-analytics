...
public abstract class PubSubNotificationToGCSUriDoFn extends DoFn<PubsubMessage, String> {
  ...
  public void processElement(ProcessContext c) {
    ...
    VideoAnalyticsPipeline.totalFiles.inc();
    ...
    if (contentType != null && !supportedContentTypes().contains(contentType)) {
      VideoAnalyticsPipeline.rejectedFiles.inc();
      LOG.warn("File {} is rejected - content type '{}' is not supported. "
              + "Refer to https://cloud.google.com/video-intelligence/docs/supported-formats for details.",
...
//   */
//  private JsonNode getMetadata(PubsubMessage message) {
//    try {
//      JsonNode payloadJson = new ObjectMapper().readTree(message.getPayload());
//      return payloadJson.get("metadata");
//    } catch (IOException e) {
//      LOG.warn("Failed to parse pubsub payload: ", e);
//      return null;
//    }
